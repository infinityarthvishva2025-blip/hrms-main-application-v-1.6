...
export const get = asyncHandler(async (req , res) => {
    ...
    const $options = {page:parseInt(page) , limit:{createdAt:-1} , populate:{path:"createdBy" , select:"name email"}}

    const result = await Video.paginate(filter , options);
    return res.status(200).json(new ApiResponse(200 , result , 'Videos Fetched succesfully'));
});
...
export const getVideoBy = asyncHandler(async (req , res) => {
    const {id} = req.params;
    ...
    // get sections with their  sunbections
...
export const create = asyncHandler(async (req , res) => {
    ...
    const uploadResult = await uploadToCloudinary(req.file.buffer , {
        folder:'gurukul/videos' , resourceType:'video'
    });

    const video = await Video.create({title , description , cloudinaryUrl:uploadResult.secure_url , publicId:uploadResult.public_id , duration:uploadResult.duration , thumbnail:uploadResult.thumbnail_url,
        createdBy:req.user._id
    });

    return res.status(201).json(new ApiResponse(201 , video , 'video created succesfully'));
